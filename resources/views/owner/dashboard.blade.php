<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Owner Dashboard') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">

            <!-- Greeting Section -->
            <div class="mb-6 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h1 class="text-3xl font-bold text-gray-900">
                    Good {{ now()->hour < 12 ? 'Morning' : (now()->hour < 18 ? 'Afternoon' : 'Evening') }}, Owner 👋
                </h1>

                <div class="mt-4 flex gap-8">
                    <div class="text-gray-600">
                        <p class="text-sm font-semibold">{{ now()->format('l') }}</p>
                        <p class="text-lg font-bold">{{ now()->format('d F Y') }}</p>
                    </div>

                    <div class="text-gray-600">
                        <p class="text-sm font-semibold">Current Time</p>
                        <p class="text-lg font-bold" id="current-time">{{ now()->format('h:i A') }}</p>
                    </div>
                </div>
            </div>

            <!-- Dashboard Cards -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">

                <!-- Upcoming Check-outs -->
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm">Upcoming Check-outs</p>
                            <p class="text-3xl font-bold text-gray-900">{{ $upcomingCheckouts }}</p>
                        </div>

                        <i class="fas fa-calendar-alt text-3xl text-blue-500"></i>
                    </div>
                </div>

                <!-- Pending Tasks -->
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm">Pending Tasks</p>
                            <p class="text-3xl font-bold text-gray-900">{{ $cleaningInProgress }}</p>
                        </div>

                        <i class="fas fa-broom text-3xl text-orange-500"></i>
                    </div>
                </div>

                <!-- Completed Today -->
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm">Completed Today</p>
                            <p class="text-3xl font-bold text-gray-900">{{ $completedToday }}</p>
                        </div>

                        <i class="fas fa-check-circle text-3xl text-green-500"></i>
                    </div>
                </div>

            </div>

            <!-- Quick Actions -->
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">

                <h3 class="text-lg font-semibold text-gray-900 mb-4">
                    Quick Actions
                </h3>

                <div class="flex flex-wrap gap-4">

                    <a href="{{ route('owner.tasks') }}"
                       class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-3 rounded-lg transition">
                        📋 View Tasks
                    </a>

                    <a href="{{ route('owner.tasks.approval') }}"
                       class="bg-green-600 hover:bg-green-700 text-white font-semibold px-5 py-3 rounded-lg transition">
                        ✅ Task Approval
                    </a>

                    <a href="{{ route('owner.tasks.history') }}"
                       class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-3 rounded-lg transition">
                        📜 Task History
                    </a>

                </div>

            </div>

            <!-- Main Content -->
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">
                    Welcome to TeratakClean Owner Dashboard
                </h3>

                <p class="text-gray-600">
                    Monitor your housekeeping operations from here.
                </p>
            </div>

        </div>
    </div>

    <script>
        function updateTime() {
            const now = new Date();
            const timeString = now.toLocaleTimeString('en-US', {
                hour: '2-digit',
                minute: '2-digit',
                hour12: true
            });

            document.getElementById('current-time').textContent = timeString;
        }

        setInterval(updateTime, 1000);
    </script>

</x-app-layout>