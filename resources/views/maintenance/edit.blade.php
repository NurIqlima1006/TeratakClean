<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit: {{ $maintenanceTask->task_name }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('maintenance.update', $maintenanceTask->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        >
                            ...
                            @foreach($units as $unit)
                                <option value="{{ $unit->id }}" {{ old('unit_id', $maintenanceTask->unit_id) === (string)$unit->id ? 'selected' : '' }}>
                                    {{ $unit->unit_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-6">
                        ...
                        <input 
                            ...
                            id="task_name" 
                            name="task_name" 
                            value="{{ old('task_name', $maintenanceTask->task_name) }}"
                            placeholder="e.g., Fix toilet lamp"
                            style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 16px;"
                        >
                        @error('task_name')
                            <p style="color: #ef4444; font-size: 14px; margin-top: 4px;">{{ $message }}</p>
                    ...
                    <div class="mb-6">
                        ...
                        >{{ old('description', $maintenanceTask->description) }}</textarea>
                        @error('description')
                            <p style="color: #ef4444; font-size: 14px; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        >
                            ...
                            @foreach($workers as $worker)
                                <option value="{{ $worker->id }}" {{ old('assigned_staff_id', $maintenanceTask->assigned_staff_id) === (string)$worker->id ? 'selected' : '' }}>
